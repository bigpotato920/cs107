...
static void BuildIndices(const char *feedsFileName, hashset *database,
        hashset *stoplist);
static void ProcessFeed(const char *remoteDocumentName, 
        hashset *database, hashset *stoplist);
static void PullAllNewsItems(urlconnection *urlconn, 
        hashset *database, hashset *stoplist);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *database,
        hashset *stoplist);
...
static const int KNumBuckets = 1009;
...
static int  data_entry_compare(const void *elem1, const void *elem2)
{
...
{
    data_entry *entry = (data_entry*)elem;
    free(entry->word);
    VectorDispose(&entry->urls);
}

static int data_entry_hash(const void *elem, int num_buckets)
{
    data_entry *entry = (data_entry*)elem;
...
int main(int argc, char **argv)
{
    ...
    HashSetNew(&database, sizeof(data_entry), KNumBuckets,
            data_entry_hash, data_entry_compare, data_entry_free);

    build_stoplist(&stoplist, kStopWordsFile);
...
static void ProcessFeed(const char *remoteDocumentName, hashset *database,
        hashset *stoplist)
{
    url u;
    ...
    switch (urlconn.responseCode) {
        case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
                break;
        case 200: PullAllNewsItems(&urlconn, database, stoplist);
                  break;
        case 301: 
        case 302: ProcessFeed(urlconn.newUrl, database, stoplist);
                  break;
...
static void PullAllNewsItems(urlconnection *urlconn, hashset *database,
        hashset *stoplist)
{
    streamtokenizer st;
    ...
    while (GetNextItemTag(&st)) { // if true is returned, then assume that <item ...> has just been read and pulled from the data stream
        ProcessSingleNewsItem(&st, database, stoplist);
    }
        ...
        hashset *stoplist)
{
    ...
    ParseArticle(articleTitle, articleDescription, articleURL, database, stoplist);
}
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset *database, hashset *stoplist)
{
    ...
    switch (urlconn.responseCode) {
        ...
        case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
                  ParseArticle(articleTitle, articleDescription, urlconn.newUrl, database, stoplist);
                  break;
            ...
            if (WordIsWellFormed(word)) {
                ...
                if ((HashSetLookup(stoplist, &m_word) == NULL)) {
                    ...
                    if (entry == NULL) {
                        ...
                        HashSetEnter(database, &m_entry);

                    } else {
                        // get the entry and push the new url into the entry
                        vector *urls = &entry->urls;
                        VectorAppend(urls, &url);
                        free(m_word);
                    }      
...
static void build_stoplist(hashset *stoplist, const char *stop_word_filename)
{
    FILE *infile;
    streamtokenizer st;
    char buffer[1024];
    ...
    infile = fopen(stop_word_filename, "r");
    assert(infile != NULL);    
